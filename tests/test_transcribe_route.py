...
def _fake_provider(name: str, *, text: str = "hello",
                   raise_: STTError | None = None) -> STTProvider:
    class Fake(STTProvider):
        async def transcribe(self, audio, mime):
            if raise_ is not None:
                raise raise_
            return TranscribeResult(text=text, language="en", duration_ms=10,
...
def test_transcribe_streaming_returns_400(app_client):
    body = {"audio_b64": base64.b64encode(b"\x00\x00").decode(),
            "mime": "audio/wav", "prefer": "streaming"}
    r = app_client.post("/v1/transcribe", json=body)
    assert r.status_code == 400


def test_transcribe_bad_base64_returns_400(app_client):
    r = app_client.post("/v1/transcribe", json={
        "audio_b64": "!!!not-base64!!!", "mime": "audio/wav"
    })
...
def test_transcribe_default_calls_registered_provider(app_client):
    ...
    body = {"audio_b64": base64.b64encode(b"\x00" * 100).decode(),
            "mime": "audio/wav", "prefer": "default"}
    r = app_client.post("/v1/transcribe", json=body)
...
def test_transcribe_provider_error_becomes_502(app_client):
    register_test_provider(
        "groq_whisper",
        _fake_provider("groq_whisper",
                        raise_=STTError("groq HTTP 500: upstream is down")),
    )
    body = {"audio_b64": base64.b64encode(b"\x00").decode(),
            "mime": "audio/wav", "prefer": "default"}
    r = app_client.post("/v1/transcribe", json=body)
