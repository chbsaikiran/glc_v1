...
def _event_callback(*, user: str, text: str, ts: str = "1700000000.000100",
    ...
    return {
        ...
        "authorizations": [{"team_id": TEAM_ID, "user_id": "USELF",
...
class SlackMock:
    ...
    def queue_threaded_message(self, text: str = "in thread",
                               thread_ts: str = "1700000000.000050") -> dict[str, Any]:
        ev = _event_callback(user=OWNER_SLACK_ID, text=text,
                             ts=self._next_ts(), thread_ts=thread_ts)
        self.inbound_events.append(ev)
        return ev
