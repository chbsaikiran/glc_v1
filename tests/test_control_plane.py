...
def test_pair_without_token_is_unauthorized(app_client):
    r = app_client.post("/v1/control/pair",
...
def test_pair_then_confirm_round_trip(app_client, install_token):
    ...
    p = app_client.post("/v1/control/pair", headers=h,
                        json={"channel": "telegram", "channel_user_id": "1",
                              "user_handle": "me"}).json()
    assert "code" in p
    c = app_client.post("/v1/control/pair/confirm", headers=h,
                        json={"code": p["code"]})
...
def test_pair_confirm_bad_code_is_404(app_client, install_token):
    h = {"Authorization": f"Bearer {install_token}"}
    r = app_client.post("/v1/control/pair/confirm", headers=h,
                        json={"code": "000000"})
...
def test_presence_returns_uptime_and_pairings(app_client, install_token):
    h = {"Authorization": f"Bearer {install_token}"}
    p = app_client.post("/v1/control/pair", headers=h,
                        json={"channel": "discord", "channel_user_id": "U1"}).json()
    app_client.post("/v1/control/pair/confirm", headers=h,
                    json={"code": p["code"]})
...
def test_pair_bad_trust_level_400(app_client, install_token):
    h = {"Authorization": f"Bearer {install_token}"}
    r = app_client.post("/v1/control/pair", headers=h,
                        json={"channel": "x", "channel_user_id": "1",
